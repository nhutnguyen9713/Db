...
async function exportCombinedPlanToExcel(){
  ...
  // Remark: báo hàng tồn kho 3A của mã này có nằm ở vị trí "trên lầu" hay không — 2 kiểu locator
  // riêng của kho 3A: "3AFG-M1xx"/"3AFG-M2xx" (số ngay sau M là tầng, VD 3AFG-M101 -> lầu M1) ghi rõ
  // đúng tầng; "3A-##-##" (2 nhóm số, không có "FG-M") chỉ ghi chung "Hàng trên lầu" (không rõ tầng).
  ...
  const computeKho3ARemark = (r) => {
    ...
    locs.forEach(l => {
      ...
      if(/^3A-\d{2}-\d{2}/i.test(loc)) hasGeneric = true;
    });
    const parts = [];
    if(floors.size) parts.push('Hàng trên lầu ' + [...floors].sort().join(', '));
    if(hasGeneric) parts.push('Hàng trên lầu');
    return parts.join('; ');
  };
...
/* ============ SƠ ĐỒ RACK 3A ============
   Chỉ gồm locator dạng "3A-##-T#" (VD: 3A-C12-T1) thuộc Kho 3A — mỗi dòng GI No. tính là 1 pallet.
   Danh sách đầy đủ 340 vị trí lấy từ sheet "Sheet2" của file layout rack (20260822 - Layout_rack.xlsx)
   — luôn hiển thị đủ tất cả vị trí trên sơ đồ, kể cả vị trí hiện chưa có pallet nào. */
const RACK3A_LOCATOR_REGEX = /^3A-[A-Za-z0-9]+-T\d+$/i;
...
function computeRack3AByLocator(){
  ...
  if(currentData){
    ...
    for(const row of raw){
      ...
      const locator = row[RAW_KEY_IDX.locator] || '';
      if(!RACK3A_LOCATOR_REGEX.test(locator)) continue;
      (map[locator] || (map[locator] = [])).push(row);
    }
  }
